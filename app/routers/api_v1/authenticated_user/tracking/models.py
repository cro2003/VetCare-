...
class DailyTrackingData(BaseModel):
    ...
    weight: Annotated[float, Field(ge=1, description="Weight in KG")]
    temperature: Annotated[float, Field(ge=1, description="Temperature in Celsius")]
    water_intake: Optional[float] = Field(ge=1, description="Water Intake in litre", default=None)
    ...
    mood_indicator: Annotated[int, Field(ge=1, description="Mood Indicator from 3")]
